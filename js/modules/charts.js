...
class ChartManager {
    ...
    this.charts.waterfall = new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [{
          ...
          data: selfCashFlow,
          backgroundColor: selfCashFlow.map(val => val >= 0 ? this.colors.positive : this.colors.negative),
          borderColor: this.colors.selfFinanced,
          borderWidth: 2
        }, {
          label: 'Bank-Financed Cash Flow',
          data: financedCashFlow,
          backgroundColor: financedCashFlow.map(val => val >= 0 ? 'rgba(33, 150, 243, 0.6)' : 'rgba(244, 67, 54, 0.6)'),
  ...
  createNetWorthProgression(results) {
    ...
    const cumulativeSelfCF = results.selfFinanced.map(year => year.cumulativeCashFlow);
    const cumulativeFinancedCF = results.financed.map(year => year.cumulativeCashFlow);
  ...
  generateSensitivityData(baseResults) {
    ...
    const baseNPV = this.calculateNPV(baseResults.selfFinanced);
    
    const parameters = [
      { name: 'Rental Rate', key: 'rentalRate', variation: 0.1 },
      { name: 'Property Appreciation', key: 'appreciationRate', variation: 0.01 },
    ...
    parameters.forEach(param => {
      ...
      const highNPV = this.calculateNPV(highResults);
      ...
      const lowNPV = this.calculateNPV(lowResults);

      sensitivityResults.labels.push(param.name);
      sensitivityResults.positive.push(Math.max(highNPV - baseNPV, lowNPV - baseNPV, 0));
      sensitivityResults.negative.push(Math.min(highNPV - baseNPV, lowNPV - baseNPV, 0));
    });
  ...
  performQuickCalculation(params) {
    ...
    for (let year = 1; year <= 15; year++) {
      const rental = params.initialCost * (params.rentalRate / 100) * Math.pow(1 + (params.appreciationRate || 3) / 100, year - 1);
      ...
      results.push({
        ...
        netWorth: params.initialCost * Math.pow(1 + (params.appreciationRate || 3) / 100, year)
      });
    }
  ...
  updateKPITiles(results) {
    ...
    this.updateKPI('self-roi', this.calculateROI(results.selfFinanced, results.inputParams));
    ...
    this.updateKPI('financed-roi', this.calculateROI(results.financed, results.inputParams));

    // Update comparison metrics
    const netWorthDiff = selfFinal.netWorth - financedFinal.netWorth;
    const cashFlowDiff = selfFinal.cumulativeCashFlow - financedFinal.cumulativeCashFlow;
  ...
  updateAlerts(results) {
    ...
    const negativeYears = results.selfFinanced.filter(year => year.cashFlow < 0).length;
    if (negativeYears > 0) {
      alerts.push({
        type: 'warning',
    ...
    const financedNegativeYears = results.financed.filter(year => year.cashFlow < 0).length;
    ...
    if (selfBreakEven > 10) {
      alerts.push({
        type: 'info',
        message: `Self-financed strategy breaks even after year ${selfBreakEven + 1}`
      });
    }
