...
class WorkerManager {
  ...
  // Initialize worker
  initWorker() {
    // Check if we can use web workers (fails in file:// protocol)
    try {
      if (this.worker) {
      ...
      console.warn('Web Workers not available (likely due to file:// protocol), using main thread calculations');
      this.worker = null;
    }
  }
  ...
  handleWorkerMessage(data) {
    const { type, data: result, error, progress } = data;

    switch (type) {
      case 'CALCULATION_COMPLETE':
        this.isCalculating = false;
        this.hideLoadingIndicator();
        utils.calculationResults = result;
        uiManager.updateAllTables();
        chartManager.initializeCharts(result);
      ...
      case 'SENSITIVITY_COMPLETE':
        this.isCalculating = false;
        this.hideLoadingIndicator();
        this.handleSensitivityResults(result);
        break;

      case 'MONTE_CARLO_COMPLETE':
        this.isCalculating = false;
        this.hideLoadingIndicator();
        this.handleMonteCarloResults(result);
        break;
      ...
      case 'ERROR':
        ...
        this.showError('Calculation error: ' + error);
        break;
  ...
  // Calculate scenarios using web worker or main thread
    ...
    try {
      ...
      setTimeout(() => {
        try {
          ...
          // Update UI components step by step with error handling
          try {
            if (window.uiManager && typeof uiManager.updateAllTables === 'function') {
              uiManager.updateAllTables();
            } else {
              console.warn('uiManager.updateAllTables not available, using legacy function');
          ...
          try {
            if (window.chartManager && typeof chartManager.initializeCharts === 'function') {
              chartManager.initializeCharts(results);
            } else {
              console.warn('Chart manager not available, skipping charts');
            }
          } catch (chartError) {
          ...
          this.isCalculating = false;
          this.hideLoadingIndicator();
          console.error('Main thread calculation error:', error);
      ...
      this.isCalculating = false;
      this.hideLoadingIndicator();
      console.error('Main thread calculation setup error:', error);
  ...
  performSensitivityAnalysisMainThread(baseParams, parameters) {
    setTimeout(() => {
      try {
        const results = this.performSimplifiedSensitivityAnalysis(baseParams, parameters);
        this.isCalculating = false;
        this.hideLoadingIndicator();
  ...
  performMonteCarloSimulation(baseParams, numSimulations = 100) {
    ...
    const variableParams = [
      { key: 'appreciationRate', mean: baseParams.appreciationRate, stdDev: 1.0 },
    ...
    this.isCalculating = true;
    this.showLoadingIndicator(`Running Monte Carlo simulation (${numSimulations} runs)...`, true);
    ...
    if (this.worker) {
      ...
      this.performMonteCarloMainThread(baseParams, numSimulations, variableParams);
    }
  }
  ...
  performMonteCarloMainThread(baseParams, numSimulations, variableParams) {
    setTimeout(() => {
      try {
        const results = this.performSimplifiedMonteCarloSimulation(baseParams, numSimulations, variableParams);
        this.isCalculating = false;
        this.hideLoadingIndicator();
  ...
  performSimplifiedMonteCarloSimulation(baseParams, numSimulations, variableParams) {
    const results = {
      simulations: [],
      statistics: {}
    };
  ...
  handleMonteCarloResults(results) {
    ...
    this.showSuccess(`Monte Carlo simulation completed (${results.simulations.length} simulations)`);
  }
  ...
  displaySensitivityResults(results) {
    ...
    results.forEach(result => {
      ...
      result.scenarios.forEach(scenario => {
        const impactClass = scenario.netWorth > 0 ? 'positive' : scenario.netWorth < 0 ? 'negative' : 'neutral';
        html += `<td class="${impactClass}">$${(scenario.netWorth/1000).toFixed(0)}K</td>`;
      });
        ...
        <div class="statistics-grid">
          <div class="stat-card">
            ...
            <div class="stat-row">
              ...
              <span>$${(statistics.netWorth.percentile5/1000).toFixed(0)}K</span>
            </div>
            <div class="stat-row">
              <span>95th Percentile:</span>
              <span>$${(statistics.netWorth.percentile95/1000).toFixed(0)}K</span>
            </div>
          </div>
          ...
          <div class="stat-card">
            ...
            <div class="stat-row">
              <span>5th Percentile:</span>
              <span>$${(statistics.cashFlow.percentile5/1000).toFixed(0)}K</span>
            </div>
            <div class="stat-row">
              <span>95th Percentile:</span>
              <span>$${(statistics.cashFlow.percentile95/1000).toFixed(0)}K</span>
            </div>
          </div>
  ...
  createDistributionCharts(results) {
    ...
    console.log('Distribution charts would be created here', results.statistics);
  }
  ...
  showLoadingIndicator(message = 'Calculating...', showProgress = false) {
    ...
    indicator.innerHTML = `
      <div class="loading-content">
        ...
        ${showProgress ? '<div class="progress-bar"><div class="progress-fill" id="progress-fill"></div></div>' : ''}
      </div>
    `;
